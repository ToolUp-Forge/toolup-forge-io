...
The server is Giraffe over ASP.NET Core. The client is Fable + Feliz with an in-tree Elmish runtime. The build is FAKE. All F#. Shared records and discriminated unions cross the client/server boundary via ToolUp.Remoting — an in-tree transport [forked from Zaid Ajaj's Fable.Remoting](https://github.com/Zaid-Ajaj/Fable.Remoting) under MIT, ships inside `ToolUp.Platform.{Core,Client,Server}`. No DTO duplication, no hand-rolled JSON, no contract-first generation step. The function signature on the server is the function signature on the client.
...
// Client.fs — typed proxy auto-generated by ToolUp.Remoting.Client
